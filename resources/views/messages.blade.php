    ...
    <main class="col dashboard-content p-4">
        ...
        <div class="row gap-3">
            <div class="col p-4">
                ...
                @endphp
                {{-- <p>{{ $conversation->id }}</p> --}}
                <a href="{{ route('conv_messages',['id'=>$conversation->id]) }}" class="message-container {{ $firstMessage?->user_id == auth()->guard('web')->id() ? 'sent' : 'received' }}">
                    <div class="message-header">
                        <img src="{{ $conversation->sender->profile_photo_url ?? asset('works/1752560823.jpg') }}"
                            alt="Sender" class="avatar" width="40" height="40">
                        <span>{{ $conversation->sender->f_name ?? 'Unknown' }}</span>
                    </div>
