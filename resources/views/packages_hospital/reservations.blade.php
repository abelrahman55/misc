        ...
        <div class="row gap-3">
            <div class="col p-4">
                ...
                @foreach ($conversations as $conversation)
                    ...
                    @endphp
                    {{--  <p>{{ $conversation->id }}</p>  --}}
                    <a href="{{ route('provider_conv_messages',['id'=>$conversation->id]) }}" class="message-container {{ $firstMessage?->user_id == auth()->id() ? 'sent' : 'received' }}">
                    ...
                    <div style="display: flex;align-items: center;justify-content: end" class="make-price">
                        <a class="btn btn-primary my-2" href="{{ route('make_provider_price',['id'=>$conversation->id]) }}">
                        عمل عرض سعر له
                    </a>
                    </div>
