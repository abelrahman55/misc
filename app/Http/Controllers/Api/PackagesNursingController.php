...
class PackagesNursingController extends Controller
{
    ...
    public function book_packages(){
        ...
        $packages=PackageNursing::with('optionsNursing')->get()
        ->map(function($item)use($lang){
            return [
                'id'=>$item->id,
                'price'=>$item->price??0,
                'title' => $item?->title[$lang] ?? "",
                'options'=>$item?->optionsNursing->map(function($it)use($lang){
                    return [
                        'id'=>$it->id??0,
                        'title'=>$it?->title[$lang]??""
                    ];
    ...
    public function other_options(){
        $lang=request()->header('lang','ar');
        $pack_id=request('id');
        $options=PackageNursingOption::whereDoesntHave('pacakesNursing',function($q)use($pack_id){
            $q->where('package_nursing_id',$pack_id);
        })->get()
                ->map(function($item)use($lang){
            return [
                'id'=>$item->id,
