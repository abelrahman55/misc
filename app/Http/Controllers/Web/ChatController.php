...
class ChatController extends Controller
{
    ...
    //             $q->where('user_id', $id)
    //               ->where('receiver_id', Auth::guard('web')->id());
    //         })
    //         ->get();
    ...
    public function index(Request $request)
    {
        ...
        $conversations = Conversation::with(['sender', 'receiver', 'messages'])
            ...
            ->orWhere('receiver_id', Auth::guard('web')->id())
            ->latest('updated_at')
            ->get();
        ...
        if ($id) {
            ...
            if ($currentConversation) {
                // Newest messages first for column-reverse
    ...
    public function show($id)
    {
        ...
        $messages = $conversation->messages?->sortByDesc('created_at')->values() ?? collect();
        // return $messages;
        if ($conversation->user_id != Auth::guard('web')->id() && $conversation->receiver_id != Auth::guard('web')->id()) {
            abort(403);
        }
